...
to setup-foods
  ...
  set food-list [ 100 75 50 1 54 48 40 32 24 16 ]
  ...
  repeat count foods [
    ask food i [
      ...
      set label quality
      set food-size item i food-list
      ;print "TAILLE DE LA BOUUFFE"
      ;print food-size
    ]
    set i i + 1
  ]
...
to dance
  set dance-task [ ->
    ifelse count bees with [piping?] in-radius 3 > 0 [
      ...
      set color [color] of target
      set piping? true
      set food-size food-size - 1
      ;print "FOOD-SIZE"
      show food-size
      ask target [set quality quality - 1
      set label quality
